...
def _clone(message, bot):
    ...
    if get_bot_pm(user_id) and message.chat.type != 'private':
        try:
            ...
            buttons.buildbutton("استارت ربات ♻️", f"{botstart}")
            startwarn = f"👤 کاربر {uname},\n\n<b>برای استفاده از امکانات ابتدا ربات را استارت کنید 🙂.</b>\n\n"
    ...
    if user_id != OWNER_ID and not is_sudo(user_id) and not is_paid(user_id):
        if config_dict['PAID_SERVICE'] is True:
            if TOTAL_TASKS_LIMIT == total_task:
                return sendMessage(f"<b>محدودیت تسک : {TOTAL_TASKS_LIMIT}\nتسک های فعال شما : {total_task}\n⛔️ به محدودیت تسک رسیدید تا اتمام تسک های قبلی صبر کنید </b>\n ", bot ,message)
            if USER_TASKS_LIMIT == get_user_task(user_id):
                return sendMessage(f"<b>محدودیت تسک : {USER_TASKS_LIMIT} \nتسک های فعال شما : {get_user_task(user_id)}\n⛔️ به محدودیت تسک رسیدید تا اتمام تسک های قبلی صبر کنید</b>\n ", bot ,message)
        else:
            if TOTAL_TASKS_LIMIT == total_task:
                return sendMessage(f"<b>محدودیت تسک : {TOTAL_TASKS_LIMIT}\nتسک های فعال شما : {total_task}\n⛔️ به محدودیت تسک رسیدید تا اتمام تسک های قبلی صبر کنید </b>", bot ,message)
            if USER_TASKS_LIMIT == get_user_task(user_id):
                return sendMessage(f"<b>محدودیت تسک : {USER_TASKS_LIMIT} \nتسک های فعال شما : {get_user_task(user_id)}\n⛔️ به محدودیت تسک رسیدید تا اتمام تسک های قبلی صبر کنید</b>", bot ,message)
...
def start_clone(listelem):
    ...
    if config_dict['STOP_DUPLICATE'] and IS_USRTD == False:
        ...
        if smsg:
            ...
            if tegr:
                return sendMessage("نام فایل ارسالی شما از قبل وجود دارد ⚠️.\nبرای مشاهده فایل ها کلیک کنید ⬇️:", bot, message, button)
            elif html:
                return sendFile(bot, message, button, f"نام فایل ارسالی شما از قبل وجود دارد ⚠️.\nبرای مشاهده فایل ها کلیک کنید ⬇️:\n\n{smsg}")
        ...
        if BOT_PM_X:
            if message.chat.type != 'private':
                if config_dict['EMOJI_THEME']:
                    msg = f"<b>🗂️ نام: </b><{config_dict['NAME_FONT']}>{escape(name)}</{config_dict['NAME_FONT']}>\n"
                else:
                    msg = f"<b>Name: </b><{config_dict['NAME_FONT']}>{escape(name)}</{config_dict['NAME_FONT']}>\n"
                botpm = f"\n<b>👤 کاربر {tag}!, !, فایل را برایتان ارسال کردیم ✅</b>\n"
                buttons = ButtonMaker()
                b_uname = bot.get_me().username
                botstart = f"http://t.me/{b_uname}"
                buttons.buildbutton("مشاهده 👁‍🗨", f"{botstart}")
                if config_dict['PICS']:
    ...
    tag = f"@{message.from_user.username}"
    if config_dict['EMOJI_THEME']:
        slmsg = f"╭🗂️ نام: <{config_dict['NAME_FONT']}>{escape(name)}</{config_dict['NAME_FONT']}>\n"
        slmsg += f"├📐 حجم: {get_readable_file_size(size)}\n"
        slmsg += f"╰👥 توسط: {tag} | <code>{user_id}</code>\n\n"
    else:
    ...
    if BOT_PM_X and message.chat.type != 'private':
        if config_dict['EMOJI_THEME']:
            pmwarn = f"<b>😉فایل برایتان ارسال شد.</b>\n"
        else:
