...
class MirrorLeechListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        ...
        if config_dict['EMOJI_THEME']:
            slmsg = f"🖊 نام: <{NAME_FONT}>{escape(name)}</{NAME_FONT}>\n\n"
            slmsg += f"📦 حجم: {size}\n"
            slmsg += f"👤 کاربر: {self.tag} | <code>{self.user_id}</code>\n\n"
        else:
        ...
        if self.isLeech:
            ...
            if typ != 0:
                if config_dict['EMOJI_THEME']:
                    ...
                    msg += f'\n<b>├ Corrupted Files: </b>{typ}'
            if config_dict['EMOJI_THEME']:
                msg += f'\n<b>├⌛ زمان:</b> {get_readable_time(time() - self.message.date.timestamp())}'
                msg += f'\n<b>╰👤 #توسط: </b>{self.tag}\n\n'
            else: 
            ...
            if not files:
                ...
                if config_dict['LEECH_LOG_INDEXING'] and config_dict['LEECH_LOG']:
                    for i in user_data['is_leech_log']:
                        indexmsg = ''
                        for index, (link, name) in enumerate(files.items(), start=1):
                            indexmsg += f"{index}. <a href='https://t.me/kingleecher_bot'>{name}</a>\n"
            ...
            if config_dict['DISABLE_DRIVE_LINK'] and self.message.chat.type != 'private':
                ...
                buttons.buildbutton("گوگل درایو 🔐", link)
            ...
            if INDEX_URL:= INDEXURL:
                ...
                if typ == "Folder":
                    ...
                    if config_dict['VIEW_LINK']:
                        ...
                        buttons.buildbutton("🌐 مشاهده", share_urls)
                    if config_dict['SOURCE_LINK']:
                        try:
                            mesg = message_args[1]
                            if is_magnet(mesg):
        ...
        if BOT_PM_X and self.message.chat.type != 'private':
            if config_dict['EMOJI_THEME']:
                bmsg = f"<b>🗂️ نام: </b><{config_dict['NAME_FONT']}>{escape(name)}</{config_dict['NAME_FONT']}>\n"
            else:
                bmsg = f"<b>Name: </b><{config_dict['NAME_FONT']}>{escape(name)}</{config_dict['NAME_FONT']}>\n"
            botpm = f"<b>\n👤 کاربر {self.tag}!, لینک برای شما ارسال شد.</b>\n"
            buttons = ButtonMaker()
