...
class MirrorStatus:
    if config_dict['EMOJI_THEME']:
        ...
        STATUS_CLONING = "🗂 درحال کپی..."
        STATUS_QUEUEDL = "💤 صف دانلود"
        STATUS_QUEUEUP = "💤 صف اپلود"
        STATUS_PAUSED = "⏸ متوقف شده"
        STATUS_ARCHIVING = "📁 درحال فشرده سازی..."
        STATUS_EXTRACTING = "📂 درحال استخراج..."
        STATUS_SPLITTING = "✂️ درحال جداسازی..."
        STATUS_CHECKING = "🔍 درحال برسی..."
        STATUS_SEEDING = "🧪 درحال سیدینگ..."
...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [MirrorStatus.STATUS_SEEDING, MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_CONVERTING, MirrorStatus.STATUS_QUEUEDL, MirrorStatus.STATUS_QUEUEUP]:
                if config_dict['EMOJI_THEME']:
                    ...
                    msg += f"\n<b>├📊 وضعیت:</b> {get_readable_file_size(download.processed_bytes())} از {download.size()}"
                    msg += f"\n<b>├⚡️ سرعت:</b> {download.speed()}"
                    msg += f"\n<b>├⏱ زمان اتمام:</b> {download.eta()}"
                    msg += f"<b> | ⏱ سپری شده: </b>{get_readable_time(time() - download.message.date.timestamp())}"
                    msg += f"\n<b>├⛓️ منبع :</b> {download.eng()}"
                ...
                if hasattr(download, 'seeders_num'):
                    try:
                        if config_dict['EMOJI_THEME']:
                            msg += f"\n<b>├🔋 سیدرز:</b> {download.seeders_num()} | <b>🪫 لیچر:</b> {download.leechers_num()}"
                            msg += f"\n<b>├🧿 انتخاب:</b> <code>/{BotCommands.BtSelectCommand} {download.gid()}</code>"
                        else:
                ...
                if download.message.chat.type != 'private':
                    try:
                        ...
                        if config_dict['EMOJI_THEME']:
                            msg += f'\n<b>├🌐 کاربر: </b><a href="https://t.me/c/{chatid}/{download.message.message_id}">{download.message.from_user.first_name}</a> | <b>آیدی :</b> <code>{download.message.from_user.id}</code>'
                    ...
                    if config_dict['EMOJI_THEME']:
                        msg += f'\n<b>├👤 کاربر:</b> ️<code>{download.message.from_user.first_name}</code> | <b>آیدی:</b> <code>{download.message.from_user.id}</code>'
                        msg += f"\n<b>╰❌ </b><code>/{BotCommands.CancelMirror} {download.gid()}</code>"
                    else:
                ...
                if config_dict['EMOJI_THEME']:
                    ...
                    msg += f"\n<b>├⛓️ منبع:</b> <code>qBittorrent v4.4.2</code>"
                    msg += f"\n<b>├⚡️ سرعت: </b>{download.upload_speed()}"
                    msg += f"\n<b>├📤 آپلود: </b>{download.uploaded_bytes()}"
                    msg += f"\n<b>├📎 Ratio: </b>{download.ratio()}"
                    msg += f" | <b>⏱ زمان اتمام: </b>{download.seeding_time()}"
                    msg += f"\n<b>├⏳ سپری شده: </b>{get_readable_time(time() - download.message.date.timestamp())}"
                    msg += f"\n<b>╰❌ </b><code>/{BotCommands.CancelMirror} {download.gid()}</code>"
                else:
        ...
        buttons = ButtonMaker()
        buttons.sbutton("♻️ تازه سازی", "status refresh")
        buttons.sbutton("📈 وضعیت", str(THREE))
        buttons.sbutton("بستن", "status close")
        sbutton = buttons.build_menu(3)
        
        if STATUS_LIMIT and tasks > STATUS_LIMIT:
            msg += f"<b>تسک ها:</b> {tasks}\n"
            buttons = ButtonMaker()
            if config_dict['EMOJI_THEME']:
                buttons.sbutton("صفحه قبل ⬅️", "status pre")
                buttons.sbutton(f"{PAGE_NO}/{PAGES}", str(THREE))
                buttons.sbutton("➡️ صفحه بعد", "status nex")
                buttons.sbutton("♻️ تازه سازی", "status refresh")
                buttons.sbutton("بستن", "status close")
            else:
