...
class GoogleDriveHelper:
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                if config_dict['EMOJI_THEME']:
                    msg += f'<b>╭🖊 نام: </b><code>{f_name}</code>'
                    msg += f'\n<b>├📦 حجم: </b>{get_readable_file_size(self.transferred_size)}'
                    msg += f'\n<b>├🗃 فرمت: </b>Folder'
                    msg += f'\n<b>├🗂 زیرپوشه: </b>{self.__total_folders}'
                    msg += f'\n<b>├🗄 فایل ها: </b>{self.__total_files}'
                else:
                ...
                if mime_type is None:
                    mime_type = 'File'
                if config_dict['EMOJI_THEME']:
                    msg += f'\n<b>├📦 حجم: </b>{get_readable_file_size(int(meta.get("size", 0)))}'
                    msg += f'\n<b>├🗃 فرمت: </b>{mime_type}'
                else:
                ...
                if INDEX_URL := INDEXURL:
                    ...
                    if config_dict['VIEW_LINK']:
                        ...
                        buttons.buildbutton("مشاهده 👁‍🗨", urls)
            ...
            if config_dict['SOURCE_LINK']:
                buttons.buildbutton(f"🔗 منبع", link)
        except Exception as err:
            if isinstance(err, RetryError):
                LOGGER.info(f"Total Attempts: {err.last_attempt.attempt_number}")
                err = err.last_attempt.exception()
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        for drive_name, dir_id, index_url in zip(DRIVES_NAMES, DRIVES_IDS, INDEX_URLS):
            ...
            if not Title:
                if tegr:
                    msg += f'<h4>نتایج یافت شده برای {fileName}</h4>'
                elif html:
            ...
            for file in response.get('files', []):
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    if tegr:
                        ...
                        msg += f"<b><a href='{furl}'>گوگل درایو 🔐</a></b>"
                    elif html:
                        msg += '<span class="container start rfontsize">' \
                              f"<div>📁 {file.get('name')} (folder)</div>" \
                               '<div class="dlinks">' \
                              f'<span> <a class="forhover" href="{furl}">گوگل درایو 🔐</a></span>'
                    else: 
                    ...
                    if index_url:
                        ...
                        url = f'{index_url}/{url_path}/'
                        if tegr or tgdi:
                            msg += f' <b>| <a href="{url}">لینک مستقیم 🌐</a></b>'
                        elif html:
                            msg += '<span> | </span>' \
                    ...
                    if index_url:
                        ...
                        if config_dict['VIEW_LINK']:
                            ...
                            if html:
                                msg += '<span> | </span>' \
                                    f'<span> <a class="forhover" href="{index_url}/{url_path}?a=view">مشاهده</a></span>'
                            else:
        ...
        if tegr:
            ...
            cap = f"<b>نتایج {contents_count} یافت شده <i>{fileName}</i></b>"
...
╰ <b>CC :</b> <a href='tg://user?id={self.user_id}'>{bot.get_chat(self.user_id).first_name}</a>'''
    ...
    def count(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                if config_dict['EMOJI_THEME']:
                    msg += f'<b>╭🖊 نام: </b><code>{name}</code>'
                    msg += f'\n<b>├📦 حجم: </b>{get_readable_file_size(self.__total_bytes)}'
                    msg += f'\n<b>├🗃 فرمت: </b>Folder'
                    msg += f'\n<b>├🗂 زیرپوشه: </b>{self.__total_folders}'
                else:
                ...
                if config_dict['EMOJI_THEME']:
                    ...
                    msg += f'\n<b>├ Type: </b>{mime_type}'
            if config_dict['EMOJI_THEME']:
                msg += f'\n<b>├🗂️ فایل ها: </b>{self.__total_files}'
            else:
                msg += f'\n<b>├ Files: </b>{self.__total_files}'
        except Exception as err:
            if isinstance(err, RetryError):
    ...
    def cancel_download(self):
        ...
        if self.__is_downloading:
            ...
            self.__listener.onDownloadError('☑️ عملیات توسط کاربر متوقف شد ☑️')
        elif self.__is_cloning:
            LOGGER.info(f"Cancelling Clone: {self.name}")
        elif self.__is_uploading:
            LOGGER.info(f"Cancelling Upload: {self.name}")
            self.__listener.onUploadError('☑️ عملیات متوقف شد ☑️')         
